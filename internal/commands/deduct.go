...
func HandleDeduct(db *sql.DB, args []string) error {
	if len(args) < minArgsDeduct {
		return fmt.Errorf("usage: pbank deduct <fund> <currency> <amount> <title> [%s X] [%s \"...\"] [--recurring <day>]", constants.FlagCategory, constants.FlagNotes)
	}
	...
	if hasRecurring {
		// Build args for RecurringDeductWithCharge
		...
		return RecurringDeductWithCharge(db, recurringArgs)
	}

	// Standard deduct (no recurring)
